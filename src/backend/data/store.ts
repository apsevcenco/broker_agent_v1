...
export const store = {
  agents: defaultAgentDefinitions as AgentDefinition[],
  assets: [] as ManagedAsset[],
  ...
  knowledge: [
    {
      ...
      content: "Asset identity, owner details, exact location, documents and commercial terms must stay internal until admin approval is granted.",
      source: "Internal V1 rule",
      reliabilityLevel: "verified",
      tags: ["confidentiality", "approval", "off-market", "yacht-broker"],
      createdAt: now(),
      updatedAt: now()
    }
  ] as KnowledgeEntry[],
...
export function logActivity(actor: ActivityLog["actor"], action: string, details: string, agentId?: string) {
  store.activity.unshift({ id: crypto.randomUUID(), agentId, actor, action, details, createdAt: now() });
}
