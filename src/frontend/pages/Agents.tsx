...
export function AgentDetail({ slug }: { slug: string }) {
  ...
  return <>
    ...
    <section className="two-col">
      <div className="panel"><h2>Agent Memory</h2><div className="toolbar"><input placeholder="Person or company" value={memoryName} onChange={e => setMemoryName(e.target.value)} /><button onClick={addMemory}>Add</button></div>{workspace?.recent.memory.length ? workspace.recent.memory.map(item => <article className="row" key={item.id}><strong>{item.personName}</strong><span>{item.relationshipStatus}</span></article>) : <Empty text="No memory entries for this agent yet." />}</div>
